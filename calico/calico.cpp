...
PYBIND11_MODULE(_calico, m) {
  ...
  py::class_<absl::Status>(m, "Status")
      ...
      .def("message",
           [](absl::Status& self) { return std::string(self.message()); });
  ...
  py::class_<Accelerometer, std::shared_ptr<Accelerometer>, Sensor>(
      ...
      .def("EnableExtrinsicsEstimation",
           &Accelerometer::EnableExtrinsicsEstimation)
      .def("EnableIntrinsicsEstimation",
           &Accelerometer::EnableIntrinsicsEstimation)
      ...
      .def("Project", [](const Accelerometer& self,
                         const std::vector<double>& interp_times,
                         const Trajectory& sensorrig_trajectory,
                         const WorldModel& world_model) {
        const auto interp_vals =
            self.Project(interp_times, sensorrig_trajectory, world_model);
        if (!interp_vals.status().ok()) {
          throw std::runtime_error(std::string("Error: ") +
                                   std::string(interp_vals.status().message()));
        }
  ...
  py::class_<Gyroscope, std::shared_ptr<Gyroscope>, Sensor>(m, "Gyroscope")
      ...
      .def("Project", [](const Gyroscope& self,
                         const std::vector<double>& interp_times,
                         const Trajectory& sensorrig_trajectory,
                         const WorldModel& world_model) {
        const auto interp_vals =
  ...
  py::class_<CameraObservationId>(m, "CameraObservationId")
      ...
      .def("__str__",
           [](const CameraObservationId& self) {
             return "stamp: " + std::to_string(self.stamp) +
                    ", image_id: " + std::to_string(self.image_id) +
                    ", model_id: " + std::to_string(self.model_id) +
                    ", feature_id: " + std::to_string(self.feature_id);
           })
  ...
  // MultiCamera class.
  py::class_<MultiCamera, std::shared_ptr<MultiCamera>, Sensor>(m,
                                                                "MultiCamera")
      .def(py::init([](const std::unordered_set<std::string>& imagers) {
        return std::make_unique<MultiCamera>(
            absl::flat_hash_set<std::string>(imagers.begin(), imagers.end()));
      }))
      .def("SetName", &MultiCamera::SetName)
      .def("GetName", &MultiCamera::GetName)
      .def("SetSensorExtrinsics", &MultiCamera::SetSensorExtrinsics)
      .def("GetSensorExtrinsics", &MultiCamera::GetSensorExtrinsics)
      .def("SetImagerExtrinsics",
           [](MultiCamera& self, const std::string& imager,
              const Pose3d& extrinsics) {
             const auto status = self.SetImagerExtrinsics(imager, extrinsics);
             if (!status.ok()) {
               throw std::runtime_error(std::string("Error: ") +
                                        std::string(status.message()));
             }
           })
      .def("GetImagerExtrinsics",
           [](const MultiCamera& self, const std::string& imager) {
             const auto extrinsics = self.GetImagerExtrinsics(imager);
             if (!extrinsics.status().ok()) {
               throw std::runtime_error(
                   std::string("Error: ") +
                   std::string(extrinsics.status().message()));
             }
      ...
      .def("SetModel",
           [](MultiCamera& self, const std::string& imager,
             ...
             const auto status = self.SetModel(imager, camera_model);
             if (!status.ok()) {
               throw std::runtime_error(std::string("Error: ") +
                                        std::string(status.message()));
             }
      ...
      .def("GetModel",
           [](const MultiCamera& self, const std::string& imager) {
             const auto model = self.GetModel(imager);
             if (!model.status().ok()) {
               throw std::runtime_error(std::string("Error: ") +
      ...
      .def("SetLatency",
           [](MultiCamera& self, const std::string& imager, double latency) {
             const auto status = self.SetLatency(imager, latency);
             if (!status.ok()) {
               throw std::runtime_error(std::string("Error: ") +
                                        std::string(status.message()));
             }
      ...
      .def(
          "GetLatency",
          [](const MultiCamera& self, const std::string& imager) {
            const auto latency = self.GetLatency(imager);
            if (!latency.status().ok()) {
              throw std::runtime_error(std::string("Error: ") +
                                       std::string(latency.status().message()));
            }
            return latency.value();
          })
      .def("EnableExtrinsicsEstimation",
           py::overload_cast<bool>(&MultiCamera::EnableExtrinsicsEstimation))
      .def("EnableImagerExtrinsicsEstimation",
           [](MultiCamera& self, const std::string& imager, bool enable) {
             const auto status =
                 self.EnableExtrinsicsEstimation(imager, enable);
             if (!status.ok()) {
               throw std::runtime_error(std::string("Error: ") +
                                        std::string(status.message()));
             }
      ...
      .def("EnableIntrinsicsEstimation",
           [](MultiCamera& self, const std::string& imager, bool enable) {
             const auto status =
                 self.EnableIntrinsicsEstimation(imager, enable);
             if (!status.ok()) {
               throw std::runtime_error(std::string("Error: ") +
                                        std::string(status.message()));
             }
           })
      .def("EnableLatencyEstimation",
           [](MultiCamera& self, const std::string& imager, bool enable) {
             const auto status = self.EnableLatencyEstimation(imager, enable);
             if (!status.ok()) {
               throw std::runtime_error(std::string("Error: ") +
                                        std::string(status.message()));
             }
           })
      .def("SetLossFunction", &MultiCamera::SetLossFunction)
      .def("SetMeasurementNoise",
           [](MultiCamera& self, double sigma) {
             const auto status = self.SetMeasurementNoise(sigma);
             if (!status.ok()) {
               throw std::runtime_error(std::string("Error: ") +
                                        std::string(status.message()));
             }
      ...
      .def("AddMeasurement",
           [](MultiCamera& self, const std::string& imager,
             ...
             const auto status = self.AddMeasurement(imager, measurement);
             if (!status.ok()) {
               throw std::runtime_error(std::string("Error: ") +
                                        std::string(status.message()));
             }
      ...
      .def("AddMeasurements",
           [](MultiCamera& self, const std::string& imager,
             ...
             const auto status = self.AddMeasurements(imager, measurements);
             if (!status.ok()) {
               throw std::runtime_error(std::string("Error: ") +
                                        std::string(status.message()));
             }
           ...
           [](MultiCamera& self) {
             std::unordered_map<
                 ...
                 std::unordered_map<CameraObservationId, CameraMeasurement,
                                    absl::Hash<CameraObservationId>>>
                 result;
             for (const auto& [imager, id_to_measurement] :
                  self.GetMeasurementIdToMeasurement()) {
               for (const auto& [id, measurement] : id_to_measurement) {
                 result[imager][id] = measurement;
               }
             }
             return result;
           })
      .def("NumberOfMeasurements", &MultiCamera::NumberOfMeasurements)
      .def("ClearMeasurements", &MultiCamera::ClearMeasurements)
      .def("Project",
           [](const MultiCamera& self, const std::vector<double>& interp_times,
              const Trajectory& sensorrig_trajectory,
              const WorldModel& world_model) {
             const auto result =
                 self.Project(interp_times, sensorrig_trajectory, world_model);
  ...
  py::class_<Trajectory, std::shared_ptr<Trajectory>>(m, "Trajectory")
      ...
      .def(
          ...
          [](Trajectory& self, const std::unordered_map<double, Pose3d>& poses,
            ...
            const auto status =
                self.FitSpline(poses_absl, knot_frequency, spline_order);
            if (!status.ok()) {
              throw std::runtime_error(std::string("Error: ") +
                                       std::string(status.message()));
            }
      ...
      .def("Interpolate",
           [](Trajectory& self, const std::vector<double>& stamps) {
             ...
             if (!poses.status().ok()) {
               throw std::runtime_error(std::string("Error: ") +
                                        std::string(poses.status().message()));
             }
             return poses.value();
           })
  ...
  py::class_<RigidBody, std::shared_ptr<RigidBody>>(m, "RigidBody")
      ...
      .def_readwrite("world_pose_is_constant",
  ...
  py::class_<WorldModel, std::shared_ptr<WorldModel>>(m, "WorldModel")
      ...
      .def("AddLandmark",
           [](WorldModel& self, std::shared_ptr<Landmark> landmark) {
             const auto status =
                 self.AddLandmark(landmark.get(), /*take_ownership=*/false);
             if (!status.ok()) {
               throw std::runtime_error(std::string("Error: ") +
                                        std::string(status.message()));
             }
      ...
      .def("AddRigidBody",
           [](WorldModel& self, std::shared_ptr<RigidBody> rigidbody) {
             const auto status =
                 self.AddRigidBody(rigidbody.get(), /*take_ownership=*/false);
             if (!status.ok()) {
               throw std::runtime_error(std::string("Error: ") +
                                        std::string(status.message()));
             }
  ...
  py::class_<ceres::Solver::Summary>(m, "Summary")
      ...
      .def_readonly("num_residuals", &ceres::Solver::Summary::num_residuals)
      .def_readonly("num_parameter_blocks",
                    &ceres::Solver::Summary::num_parameter_blocks)
      .def_readonly("num_parameters", &ceres::Solver::Summary::num_parameters)
  ...
  py::class_<AprilGridDetector>(m, "AprilGridDetector")
      ...
      .def("GetRigidBodyDefinition",
           &AprilGridDetector::GetRigidBodyDefinition);
}
